<!DOCTYPE html>
<html lang="en">
<body>
<?php include 'nav-and-footer/header-nav.php';?>

<!-- Bootstrap Grid start -->
<div class="col-12 mt-5">
                        <div class="card">
                            <div class="card-body">
                                <h4>Employee List</h4>

<!-- Start 6 column grid system -->





                                <!-- Start 12 column grid system -->
                                <div class="row">
                                    <!-- anchor tab start -->
                    <div class="col-lg-6 mt-5">
                        <div class="card">
                            <div class="card-body">
                                <nav>
                                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                        <a class="nav-link active" id="nav-home-tab" data-toggle="tab" href="#nav-group-A" role="tab" aria-controls="nav-group-A" aria-selected="true">Group A</a>
                                        <a class="nav-link" id="v-nav-profile-tab" data-toggle="tab" href="#nav-group-B" role="tab" aria-controls="nav-group-B" aria-selected="false">Group B</a>
                                        <a class="nav-link" id="v-nav-messages-tab" data-toggle="tab" href="#nav-group-C" role="tab" aria-controls="nav-group-C" aria-selected="false">Group C</a>
                                        <a class="nav-link" id="v-nav-settings-tab" data-toggle="tab" href="#nav-group-D" role="tab" aria-controls="nav-group-D" aria-selected="false">Group D</a>
                                        <a class="nav-link" id="v-nav-settings-tab" data-toggle="tab" href="#nav-group-E" role="tab" aria-controls="nav-group-E" aria-selected="false">Group E</a>
                                        <a class="nav-link" id="v-nav-settings-tab" data-toggle="tab" href="#nav-group-F" role="tab" aria-controls="nav-group-F" aria-selected="false">Group F</a>
                                        <a class="nav-link" id="v-nav-settings-tab" data-toggle="tab" href="#nav-group-G" role="tab" aria-controls="nav-group-G" aria-selected="false">Group G</a>
                                        <a class="nav-link" id="v-nav-settings-tab" data-toggle="tab" href="#nav-group-H" role="tab" aria-controls="nav-group-H" aria-selected="false">Group H</a>

                                    </div>
                                </nav>
                                <div class="tab-content mt-3" id="nav-tabContent">
                                    <div class="tab-pane fade show active" id="nav-group-A" role="tabpanel" aria-labelledby="nav-home-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-a" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                    
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                    <div class="tab-pane fade" id="nav-group-B" role="tabpanel" aria-labelledby="nav-profile-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-b" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Unavailable
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                    <div class="tab-pane fade" id="nav-group-C" role="tabpanel" aria-labelledby="nav-contact-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-c" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                    <div class="tab-pane fade" id="nav-group-D" role="tabpanel" aria-labelledby="nav-contact-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-d" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Unvailable
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                    <div class="tab-pane fade" id="nav-group-E" role="tabpanel" aria-labelledby="nav-contact-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-e" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Unvailable
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                    <div class="tab-pane fade" id="nav-group-F" role="tabpanel" aria-labelledby="nav-contact-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-f" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Unvailable
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                    <div class="tab-pane fade" id="nav-group-G" role="tabpanel" aria-labelledby="nav-contact-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-g" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Unvailable
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                    <div class="tab-pane fade" id="nav-group-H" role="tabpanel" aria-labelledby="nav-contact-tab">
                                    <div class="single-table">
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="group-h" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                    <th scope="col">Action</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Unvailable
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                    <td>
                                                        <ul class="d-flex justify-content-center">
                                                            <li class="mr-3"><a href="#" class="text-secondary"><i class="fa fa-edit"></i></a></li>
                                                            
                                                        </ul>
                                                    </td>
                                                    
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- anchor tab end -->
                    <div class="col-md-6">
                    
                    <div class="single-table">
                    <h4>Assigning</h4><br><br>
                                    <div class="table-responsive" style="table-layout: auto;">
                                        <table id="assign-table1" class="table table-hover progress-table text-center">
                                            <thead class="text-uppercase">
                                                <tr>
                                                    <th scope="col">ID</th>
                                                    <th scope="col">Name</th>
                                                    <th scope="col">Contact</th>
                                                    <th scope="col">Date</th>
                                                    <th scope="col">Status</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <th scope="row">1</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                <tr>
                                                    <th scope="row">2</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <th scope="row">3</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                                <tr>
                                                    <th scope="row">4</th>
                                                    <td>Mark</td>
                                                    <td>639521458751</td>
                                                    <td>09 / 07 / 2018</td>
                                                    <td>
                                                        Available
                                                    </td>
                                            </tbody>
                                        </table>
                                    </div><br>
                                    <div class="text-right"> <!-- This div aligns its content to the right -->
            <button class="continue-application">
                <div>
                    <div class="pencil"></div>
                    <div class="folder">
                        <div class="top">
                            <svg viewBox="0 0 24 27">
                                <path d="M1,0 L23,0 C23.5522847,-1.01453063e-16 24,0.44771525 24,1 L24,8.17157288 C24,8.70200585 23.7892863,9.21071368 23.4142136,9.58578644 L20.5857864,12.4142136 C20.2107137,12.7892863 20,13.2979941 20,13.8284271 L20,26 C20,26.5522847 19.5522847,27 19,27 L1,27 C0.44771525,27 6.76353751e-17,26.5522847 0,26 L0,1 C-6.76353751e-17,0.44771525 0.44771525,1.01453063e-16 1,0 Z"></path>
                            </svg>
                        </div>
                        <div class="paper"></div>
                    </div>
                </div>
                Assign Employee
            </button>
        </div>


                                </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- Bootstrap Grid end -->

<?php include 'nav-and-footer/footer-area.php';?> 
    
</body>
</html>


<!-- <div class="emp-list">
    <div class="emp-border"></div>
    <div class="emp-content">
      <div class="emp-logo">
        <div class="logo1">
          <svg viewBox="0 0 29.667 31.69" xmlns="http://www.w3.org/2000/svg" id="logo-main">
            <path transform="translate(0 0)" d="M12.827,1.628A1.561,1.561,0,0,1,14.31,0h2.964a1.561,1.561,0,0,1,1.483,1.628v11.9a9.252,9.252,0,0,1-2.432,6.852q-2.432,2.409-6.963,2.409T2.4,20.452Q0,18.094,0,13.669V1.628A1.561,1.561,0,0,1,1.483,0h2.98A1.561,1.561,0,0,1,5.947,1.628V13.191a5.635,5.635,0,0,0,.85,3.451,3.153,3.153,0,0,0,2.632,1.094,3.032,3.032,0,0,0,2.582-1.076,5.836,5.836,0,0,0,.816-3.486Z" data-name="Path 6" id="Path_6"></path>
            <path transform="translate(-45.91 0)" d="M75.207,20.857a1.561,1.561,0,0,1-1.483,1.628h-2.98a1.561,1.561,0,0,1-1.483-1.628V1.628A1.561,1.561,0,0,1,70.743,0h2.98a1.561,1.561,0,0,1,1.483,1.628Z" data-name="Path 7" id="Path_7"></path>
            <path transform="translate(0 -51.963)" d="M0,80.018A1.561,1.561,0,0,1,1.483,78.39h26.7a1.561,1.561,0,0,1,1.483,1.628v2.006a1.561,1.561,0,0,1-1.483,1.628H1.483A1.561,1.561,0,0,1,0,82.025Z" data-name="Path 8" id="Path_8"></path>
          </svg>
        </div>
        <div class="logo2">
          <svg viewBox="0 0 101.014 23.535" xmlns="http://www.w3.org/2000/svg" id="logo-second">
            <g transform="translate(-1029.734 -528.273)">
              <path transform="translate(931.023 527.979)" d="M109.133,14.214l3.248-11.706A1.8,1.8,0,0,1,114.114,1.2h2.229a1.789,1.789,0,0,1,1.7,2.358L111.884,21.71a1.8,1.8,0,0,1-1.7,1.216h-3a1.8,1.8,0,0,1-1.7-1.216L99.317,3.554a1.789,1.789,0,0,1,1.7-2.358h2.229a1.8,1.8,0,0,1,1.734,1.312l3.248,11.706a.468.468,0,0,0,.9,0Z" data-name="Path 1" id="Path_1"></path>
              <path transform="translate(888.72 528.773)" d="M173.783,22.535a10.77,10.77,0,0,1-7.831-2.933,10.387,10.387,0,0,1-3.021-7.813v-.562A13.067,13.067,0,0,1,164.2,5.372,9.315,9.315,0,0,1,167.81,1.4,10.176,10.176,0,0,1,173.136,0,9.105,9.105,0,0,1,180.2,2.812q2.576,2.812,2.577,7.973v.583a1.793,1.793,0,0,1-1.8,1.787H169.407a.466.466,0,0,0-.457.564,5.08,5.08,0,0,0,5.217,4.136A6.594,6.594,0,0,0,178.25,16.6a1.817,1.817,0,0,1,2.448.218l.557.62a1.771,1.771,0,0,1-.1,2.488,9.261,9.261,0,0,1-2.4,1.57,11.732,11.732,0,0,1-4.972,1.034ZM173.115,4.68A3.66,3.66,0,0,0,170.3,5.85,6.04,6.04,0,0,0,168.911,9.2h8.125V8.735a4.305,4.305,0,0,0-1.051-3,3.781,3.781,0,0,0-2.87-1.059Z" data-name="Path 2" id="Path_2"></path>
              <path transform="translate(842.947 528.771)" d="M244.851,3.928a1.852,1.852,0,0,1-1.95,1.76c-.1,0-.2,0-.3,0a7.53,7.53,0,0,0-2.234.3,3.275,3.275,0,0,0-2.348,3.1V20.347a1.844,1.844,0,0,1-1.9,1.787h-2.366a1.844,1.844,0,0,1-1.9-1.787V1.751A1.391,1.391,0,0,1,233.294.4h3.043a1.4,1.4,0,0,1,1.428,1.265l.035.533a.282.282,0,0,0,.5.138A5.617,5.617,0,0,1,242.988,0h.031a1.832,1.832,0,0,1,1.864,1.813l-.032,2.114Z" data-name="Path 3" id="Path_3"></path>
              <path transform="translate(814.555 528.773)" d="M287.2,16.127a1.869,1.869,0,0,0-1.061-1.677,12.11,12.11,0,0,0-3.406-1.095q-7.8-1.627-7.8-6.587a5.956,5.956,0,0,1,2.415-4.83A9.781,9.781,0,0,1,283.659,0a10.536,10.536,0,0,1,6.659,1.948,6.36,6.36,0,0,1,2.029,2.586,1.791,1.791,0,0,1-1.661,2.475h-2.291a1.754,1.754,0,0,1-1.624-1.137,2.7,2.7,0,0,0-.606-.922,3.435,3.435,0,0,0-2.526-.814,3.512,3.512,0,0,0-2.284.663,2.088,2.088,0,0,0-.808,1.687,1.786,1.786,0,0,0,.92,1.557,9.485,9.485,0,0,0,3.1,1.024,25.5,25.5,0,0,1,3.678.974q4.627,1.687,4.628,5.844a5.659,5.659,0,0,1-2.567,4.81,11.125,11.125,0,0,1-6.629,1.838,11.627,11.627,0,0,1-4.881-.974,8.173,8.173,0,0,1-3.345-2.671,6.843,6.843,0,0,1-.679-1.174,1.784,1.784,0,0,1,1.653-2.492h1.9a1.786,1.786,0,0,1,1.673,1.133,2.8,2.8,0,0,0,.925,1.237,4.587,4.587,0,0,0,2.87.824,4.251,4.251,0,0,0,2.536-.632,1.965,1.965,0,0,0,.859-1.657Z" data-name="Path 4" id="Path_4"></path>
              <path transform="translate(772.607 528.773)" d="M348.648,22.535a10.77,10.77,0,0,1-7.832-2.933,10.386,10.386,0,0,1-3.021-7.813v-.562a13.067,13.067,0,0,1,1.273-5.854A9.314,9.314,0,0,1,342.676,1.4,10.174,10.174,0,0,1,348,0a9.1,9.1,0,0,1,7.063,2.812q2.576,2.812,2.577,7.973v.583a1.793,1.793,0,0,1-1.8,1.787H344.272a.467.467,0,0,0-.457.564,5.081,5.081,0,0,0,5.217,4.136,6.594,6.594,0,0,0,4.083-1.251,1.817,1.817,0,0,1,2.448.218l.557.62a1.771,1.771,0,0,1-.1,2.488,9.26,9.26,0,0,1-2.4,1.57,11.731,11.731,0,0,1-4.972,1.034ZM347.981,4.68a3.659,3.659,0,0,0-2.819,1.17A6.035,6.035,0,0,0,343.777,9.2H351.9V8.735a4.307,4.307,0,0,0-1.051-3,3.781,3.781,0,0,0-2.87-1.059Z" data-name="Path 5" id="Path_5"></path>
            </g>
          </svg>
        </div>
        <span class="trail"></span>
      </div>
      <span class="logo-bottom-text">uiverse.io</span>
    </div>
    <span class="bottom-text">universe of ui</span>
  </div> -->